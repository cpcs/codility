...
void dfs(const vector<unordered_set<int>> &con, const vector<int> &D, vector<int> &mini, int x, int f, int w) {
    mini[x] = w;
    w = min(w, D[x]);
    for (int y : con[x]) {
        if (y != f) {
            dfs(con, D, mini, y, x, w);
        }
    }
}
...
int solution(int K, vector<int> &C, vector<int> &D) {
    ...
    vector<int> mini(n);
    dfs(con, D, mini, m, -1, INT_MAX);
    set<vector<int>> leaves;
    multiset<int> remain;
    for (int i = 0; i < n; ++i) {
        if (con[i].size() == 1) {
            leaves.insert({D[i], mini[i] < D[i] ? 0 : 1, i});
        }
        remain.insert(D[i]);
    }
    for (int v = INT_MIN; remain.size() > K || *remain.begin() < v;) {
        ...
        if (con[other].size() == 1) {
            leaves.insert({D[other], mini[other] < D[other] ? 0 : 1, other});
        }
    }
